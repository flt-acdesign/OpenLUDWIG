...
function setup_multilevel_domain(stl_paths::Vector{String}; num_levels=NUM_LEVELS_CONFIG)
    # Load and subdivide each STL separately; track triangle counts for object_names
    meshes = Geometry.SolverMesh[]
    triangle_counts = Int[]

    for (i, path) in enumerate(stl_paths)
        if length(stl_paths) > 1
            println("[Domain] Loading [$i/$(length(stl_paths))]: $path")
        else
            println("[Domain] Loading: $path")
        end
        if !isfile(path); error("STL file not found: $path"); end
        m = Geometry.load_mesh(path, scale=Float64(STL_SCALE),
                               min_facet_size=Float64(MINIMUM_FACET_SIZE))
        push!(meshes, m)
        push!(triangle_counts, length(m.triangles))
    end

    # Merge into a single unified mesh (no-op for single STL)
    mesh = Geometry.merge_meshes(meshes)

    # Build object_names mapping: triangle index → source STL name (without extension)
    object_names = Vector{String}(undef, length(mesh.triangles))
    offset = 0
    for (i, path) in enumerate(stl_paths)
        name = replace(basename(path), r"\.[sS][tT][lL]$" => "")
        for j in 1:triangle_counts[i]
            object_names[offset + j] = name
        end
        offset += triangle_counts[i]
    end

    # Store unrotated mesh and centroids (subdivided but not rotated)
    # for force CSV export and binary pressure export (STL-frame consistency)
    unrotated_mesh = mesh  # full mesh before rotation
    unrotated_centers = copy(mesh.centers)

    # Apply model rotation: alpha around Y, then beta around Z
    mesh = Geometry.rotate_mesh(mesh, ALPHA_DEG, BETA_DEG)
    ...
    return setup_multilevel_domain(mesh, params), mesh, unrotated_mesh, unrotated_centers, object_names
end

# Backward-compatible single-path overload
function setup_multilevel_domain(stl_path::String; num_levels=NUM_LEVELS_CONFIG)
    return setup_multilevel_domain([stl_path]; num_levels=num_levels)
end