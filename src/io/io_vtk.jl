...
function export_merged_mesh_sync(t_step, grids, out_dir, backend)
    ...
    ddf_active = DDF_ENABLED
    for lvl in 1:length(grids)
        level = grids[lvl]
        rho_cpu = Array(level.rho)
        vel_cpu = iseven(t_step) ? Array(level.vel_temp) : Array(level.vel)
        obs_cpu = Array(level.obstacle)
        if ddf_active && has_thermal(level)
            temp_cpu = Array(level.temperature)
            level_data[lvl] = (rho=rho_cpu, vel=vel_cpu, obs=obs_cpu, dx=Float32(level.dx), temperature=temp_cpu)
        else
            level_data[lvl] = (rho=rho_cpu, vel=vel_cpu, obs=obs_cpu, dx=Float32(level.dx), temperature=nothing)
        end
    end
    ...
    temp_arr = ddf_active ? Vector{Float32}(undef, n_total * n_cells) : nothing
    ...
    Threads.@threads for i in 1:n_total
        ...
        for z in 1:BLOCK_SIZE, y in 1:BLOCK_SIZE, x in 1:BLOCK_SIZE
            ...
            if temp_arr !== nothing && data.temperature !== nothing
                temp_arr[cidx] = data.temperature[x, y, z, blk.b_idx]
            end
        end
    end
    ...
    if temp_arr !== nothing
        replace!(temp_arr, NaN32 => 0f0, Inf32 => 0f0, -Inf32 => 0f0)
    end
    ...
    try
        vtk_grid(filename, points, cells; compress=true, append=false) do vtk
            ...
            if temp_arr !== nothing; vtk["Temperature"] = temp_arr; end
        end
    catch e
